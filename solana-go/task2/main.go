...
// main函数是程序的入口点，用于订阅Solana区块链上的交易签名状态
// 该函数连接到Solana测试网，订阅指定交易签名的通知，并持续接收和打印订阅消息
// 参数: 无
// 返回值: 无
func main() {
        // 创建 Solana RPC 客户端
        ...
        // 连接到Solana测试网的WebSocket服务
        client, err := ws.Connect(context.Background(), rpc.DevNet_WS)
            if err != nil {
            panic(err)
        }
    
        // 解析并创建一个必须有效的Solana交易签名对象
        txSig := solana.MustSignatureFromBase58(txResp.String())
        ...
        // 持续接收订阅消息并打印输出
        for {
            log.Println("Waiting for signature status...")
